...
import { ResponseStatus, MessageTypes, DependencyInjection, TemplateHelper, MessageHelper, useFetch } from '@/core';
import { AccountListWrapperClass } from '@/modules/accounts/constants';
import { AccountStore, Filter, Sort, FilterRole, Search, Pagination, Account, NextPage, SelectItem, AccountDelete } from '@/modules/accounts/models';
import { GetFilterValues, GetPageNav, GetAccounts, DeleteAccount, DeleteAccounts } from '@/modules/accounts/use-case';
...
export const onDeleteSelectedItems = async ({ state, dispatch }) => {
  try {
    await MessageHelper.showConfirmMessageBox({
      ...
      callback: async () => {
        const response = await fetchDeleteItems(
          new AccountDelete({
